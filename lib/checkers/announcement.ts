import { anthropic, extractJson, textOf } from "@/lib/ai/client";
...
const PATTERNS: { re: RegExp; name: string; severity: SignalSeverity; detail: string }[] = [
  { re: /\b(you'?ve been (selected|chosen|whitelisted)|congratulations)\b/i, name: "“You’ve been selected” bait", severity: "caution", detail: "Plays on excitement to rush you into acting." },
  { re: /\b(hurry|last chance|ends? (soon|today)|only \d+ (left|spots)|act now|24\s?h)/i, name: "False urgency", severity: "caution", detail: "Creates time pressure so you don't stop to verify." },
  { re: /\b(surprise|stealth)\s?mint\b/i, name: "Surprise / stealth mint", severity: "caution", detail: "Unannounced mints are frequently compromised-account scams." },
  { re: /\b(we (got|were) hacked|account (was )?compromised|use (this )?new (link|contract|server|discord))\b/i, name: "“We got hacked — use new link”", severity: "danger", detail: "Classic hijacked-announcement pattern that pushes a malicious link." },
  { re: /\b(claim|airdrop|reward|free\s?(mint|nft))\b/i, name: "Free claim / airdrop bait", severity: "caution", detail: "Free-claim framing is a common drainer lure." },
  { re: /\b(connect (your )?wallet|verify (your )?wallet|sign (to|in)|validate (your )?assets)\b/i, name: "Asks you to connect / sign to “verify”", severity: "caution", detail: "Legitimate teams never need you to sign to “verify” assets." },
];
...
function heuristicTactics(text: string): Signal[] {
  ...
  for (const p of PATTERNS) {
    if (p.re.test(text)) {
      out.push({ id: `tactic-${slug(p.name)}`, label: p.name, severity: p.severity, detail: p.detail, source: "keyword heuristic" });
    }
  }
  return out;
}
...
export async function checkAnnouncement(text: string, ctx: CheckContext): Promise<CheckerOutput> {
  ...
  // 1. Tactic analysis (Claude, with keyword fallback).
  const client = anthropic();
  if (!client) {
    degraded.push("Claude (no API key — using keyword heuristics)");
    ...
    try {
      ...
      degraded.push("Claude (analysis error — using keyword heuristics)");
      signals.push(...heuristicTactics(text));
    }
  }
